...
// +7<код><номер>, 8<код><номер>, <номер>, где <номер> — это семь цифр, а <код> — это три цифры
  ...
  const numbersArr = data
    ...
    .map((number) => {
      number = number
        ...
        .replace(/[\(\)-]/g, '')
        .slice(-10);
      if (!number.slice(-10, -7)) number = '495' + number;
      return number;
    });
    return numbersArr.slice(1).map(n => n === numbersArr[0] ? 'YES' : 'NO').join('\n');
}
...
fs.readFile('input.txt', 'utf8', (err, data) => {
  ...
  const result = isSameNumber(data);
